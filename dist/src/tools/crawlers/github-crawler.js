...
exports.parseAwesomeListMarkdown = parseAwesomeListMarkdown;
exports.crawlAwesomeList = crawlAwesomeList;
exports.diffAwesomeList = diffAwesomeList;
exports.crawlGitHubTrending = crawlGitHubTrending;
exports.processDiscoveries = processDiscoveries;
 ...
 */
function parseAwesomeListMarkdown(markdown) {
    const entries = [];
 ...
 */
async function crawlAwesomeList(config, crawlSessionId) {
    ...
    try {
        ...
        const result = await (0, fetcher_1.fetchUrl)(sourceUrl);
        if (!result.success) {
            return {
                success: false,
                entries: [],
 ...
 */
function diffAwesomeList(currentEntries, previousEntries) {
 ...
 */
function filterAIEntries(entries) {
    const aiKeywords = [
 ...
 */
async function crawlGitHubTrending(config, crawlSessionId) {
    const { languages = [''], // Empty string = all languages
    since = 'daily', } = config;
    const allEntries = [];
    const errors = [];
    ...
    for (const language of languages) {
        ...
        try {
            ...
            errors.push(`Error fetching ${language || 'all'}: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    // Log to database
    if (crawlSessionId) {
 ...
 */
async function processDiscoveries(crawlResult, projectId, sourceId) {
    const created = [];
    const skipped = [];
    const errors = [];
    for (const entry of crawlResult.entries) {
        try {
            ...
            const existing = await client_1.prisma.entity.findFirst({
                where: {
                    projectId,
                    name: entry.name,
                },
            ...
            errors.push(`Error creating ${entry.name}: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    return {
...
//# sourceMappingURL=github-crawler.js.map