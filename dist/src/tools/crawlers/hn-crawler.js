...
exports.HN_AI_KEYWORDS = void 0;
exports.crawlHackerNews = crawlHackerNews;
exports.fetchStory = fetchStory;
exports.fetchStoriesParallel = fetchStoriesParallel;
exports.fetchStoryComments = fetchStoryComments;
exports.isToolAnnouncement = isToolAnnouncement;
exports.persistDiscoveries = persistDiscoveries;
exports.crawlAndPersist = crawlAndPersist;
exports.crawlShowHN = crawlShowHN;
exports.crawlTopAIStories = crawlTopAIStories;
 ...
 */
async function crawlHackerNews(config) {
    const { endpoint, limit = 100, minScore = 10, aiKeywords = exports.HN_AI_KEYWORDS, sourceId, projectId, concurrency = 10, commentLimit = 5, } = config;
    const errors = [];
    const discoveries = [];
    try {
        ...
        const relevantStories = scoredStories.filter(story => matchesKeywords(story, aiKeywords));
 ...
 */
async function fetchStoriesParallel(ids, concurrency = 10) {
    const results = [];
    ...
    for (let i = 0; i < ids.length; i += concurrency) {
        ...
        results.push(...batchResults.filter((s) => s !== null));
    }
    return results;
}
/**
 ...
 */
async function fetchStoryComments(storyId, limit = 5) {
    try {
 ...
 */
function isToolAnnouncement(story) {
 ...
 */
function extractMatchedKeywords(story, keywords) {
    const searchText = [
        story.title,
        story.url || '',
        story.text || '',
 ...
 */
async function extractToolDiscovery(story, commentLimit) {
    ...
    return {
        ...
        matchedKeywords: extractMatchedKeywords(story, exports.HN_AI_KEYWORDS),
        context,
    };
}
/**
 ...
 */
async function persistDiscoveries(discoveries, projectId, sourceId) {
    ...
    for (const discovery of discoveries) {
        try {
            ...
            const existing = await index_1.prisma.entity.findFirst({
                where: {
                    projectId,
                    url: discovery.url,
                },
            ...
            if (existing) {
                ...
                // Create entity
                const entity = await index_1.prisma.entity.create({
                    data: {
                        projectId,
                        name: discovery.name,
                        url: discovery.url,
            ...
            const assertion = await index_1.prisma.assertion.create({
                data: {
                    entityId,
            ...
            if (sourceId) {
                await index_1.prisma.assertionSource.create({
                    data: {
 ...
 */
async function crawlAndPersist(config) {
    // Crawl HN
    ...
    // Persist discoveries
    const persistResult = await persistDiscoveries(crawlResult.discoveries, config.projectId, config.sourceId);
    return {
        ...crawlResult,
 ...
 */
async function crawlShowHN(projectId, limit = 50) {
 ...
 */
async function crawlTopAIStories(projectId, limit = 100) {
    return crawlAndPersist({
        endpoint: 'topstories',
        limit,
...
//# sourceMappingURL=hn-crawler.js.map