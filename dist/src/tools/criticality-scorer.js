...
exports.calculateCriticality = calculateCriticality;
exports.scoreEntityAssertions = scoreEntityAssertions;
exports.scoreProjectAssertions = scoreProjectAssertions;
exports.rescoreAssertionsWithNewSources = rescoreAssertionsWithNewSources;
exports.getAssertionsByCriticality = getAssertionsByCriticality;
exports.getCriticalAssertionsNeedingValidation = getCriticalAssertionsNeedingValidation;
exports.getCriticalitySummary = getCriticalitySummary;
 ...
 */
async function calculateCriticality(assertionId, weights) {
    ...
    const mentionVelocity = calculateMentionVelocity(assertion.mentionCount, assertion.firstDiscoveredAt);
    const factors = {
        federalRelevance,
        pricingImpact,
    ...
    // 3. Apply weights
    const score = federalRelevance * finalWeights.federalRelevance +
    ...
    await client_1.default.assertion.update({
        where: { id: assertionId },
        data: {
 ...
 */
function calculateFederalRelevance(claim, reasoning) {
    const allText = [claim, ...(reasoning || [])].join(' ').toLowerCase();
    const matchCount = FEDERAL_KEYWORDS.filter(keyword => allText.includes(keyword)).length;
 ...
 */
function calculatePricingImpact(claim, category) {
    let score = 0.0;
    ...
    const matchCount = PRICING_KEYWORDS.filter(keyword => claimLower.includes(keyword)).length;
    if (matchCount > 0) {
 ...
 */
function calculateSecurityArchitecture(claim) {
    const claimLower = claim.toLowerCase();
    const matchCount = SECURITY_KEYWORDS.filter(keyword => claimLower.includes(keyword)).length;
    // 0 keywords = 0.0, 1 = 0.4, 2 = 0.7, 3+ = 1.0
    if (matchCount === 0)
        return 0.0;
    if (matchCount === 1)
 ...
 */
async function calculateNovelty(entityId, claimText) {
    ...
    if (similarAssertions.length === 0)
        return 1.0;
    // Check text similarity with existing assertions
    const similarities = similarAssertions.map(other => calculateTextSimilarity(claimText, other.claim));
 ...
 */
async function calculateSourceTrust(sourceIds) {
    if (sourceIds.length === 0)
        return 0.0;
    const sources = await client_1.default.source.findMany({
        where: {
            id: { in: sourceIds },
        },
        select: {
 ...
 */
function calculateSourceSpread(sourceCount) {
    if (sourceCount === 0)
        return 0.0;
    if (sourceCount === 1)
        return 0.3;
 ...
 */
function calculateMentionVelocity(mentionCount, firstDiscoveredAt) {
    if (!firstDiscoveredAt || mentionCount <= 1)
        return 0.0;
    const daysSinceDiscovery = Math.max(1, Math.floor((Date.now() - firstDiscoveredAt.getTime()) / (1000 * 60 * 60 * 24)));
    // Mentions per day
 ...
 */
function generateExplanation(level, factors, weights) {
    const parts = [];
    // Overall assessment
 ...
 */
async function scoreEntityAssertions(entityId, weights) {
 ...
 */
async function scoreProjectAssertions(projectId, weights) {
    const assertions = await client_1.default.assertion.findMany({
        where: {
            entity: { projectId },
            status: { not: 'REJECTED' },
 ...
 */
async function rescoreAssertionsWithNewSources(assertionIds) {
 ...
 */
async function getAssertionsByCriticality(projectId, level, limit) {
 ...
 */
async function getCriticalAssertionsNeedingValidation(projectId) {
    return client_1.default.assertion.findMany({
        where: {
            entity: { projectId },
            status: 'CLAIM',
            criticality: {
                in: [client_2.AssertionCriticality.CRITICAL, client_2.AssertionCriticality.HIGH],
            },
        },
        include: {
            entity: {
 ...
 */
async function getCriticalitySummary(projectId) {
    const assertions = await client_1.default.assertion.findMany({
        where: {
            entity: { projectId },
            status: { not: 'REJECTED' },
    ...
    for (const assertion of assertions) {
        // Count by level
        if (assertion.criticality === client_2.AssertionCriticality.CRITICAL) {
            byLevel.CRITICAL++;
            if (assertion.status === 'CLAIM')
                needingValidation++;
        }
        else if (assertion.criticality === client_2.AssertionCriticality.HIGH) {
            byLevel.HIGH++;
            if (assertion.status === 'CLAIM')
                needingValidation++;
        }
        else if (assertion.criticality === client_2.AssertionCriticality.MEDIUM) {
            byLevel.MEDIUM++;
        }
        else if (assertion.criticality === client_2.AssertionCriticality.LOW) {
            byLevel.LOW++;
        }
    }
    return {
...
//# sourceMappingURL=criticality-scorer.js.map