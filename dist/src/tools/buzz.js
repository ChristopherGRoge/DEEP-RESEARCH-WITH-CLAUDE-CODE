 ...
 */
async function calculateConceptCoverage(entityId, categoryId) {
    const missing = [];
    if (!categoryId) {
    ...
    const totalConcepts = await client_1.prisma.categoryConcept.count({
        where: { categoryId },
    });
    if (totalConcepts === 0) {
...
async function calculateBuzzScore(input) {
    ...
    const baseScore = components.marketPresence * 0.30 +
