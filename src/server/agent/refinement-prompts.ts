 ...
 * by human validators.
 */
export const REFINEMENT_SYSTEM_PROMPT = `You are a research refinement specialist in a Deep Research system that helps federal agencies evaluate AI coding tools.
 ...
 * or refute a claim.
 */
export const EVIDENCE_COLLECTION_SYSTEM_PROMPT = `You are an evidence collector in a Deep Research system that helps federal agencies evaluate AI coding tools.
 ...
 */
export function buildEvidencePrompt(request: EvidenceRequest): string {
  const {
    claim,
    entityUrl,
    entityName,
 ...
 */
export function buildDefenseExplanationPrompt(
 ...
 */
export function buildRefinementSummaryPrompt(
  assertionId: string,
  originalClaim: string,
  revisedClaim: string | null,
  action: 'revised' | 'defended' | 'uncertain'
): string {
