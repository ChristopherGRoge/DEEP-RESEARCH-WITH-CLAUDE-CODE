...
import { researchSessionManager, type ResearchCategory } from '../agent/research-session';
import { ResearchSessionStatus, ResearchTaskStatus, AssertionStatus } from '../../../generated/prisma/client';
import { FeedbackType, type AssertionFeedback, type FeedbackResult } from '../agent/feedback-types';
import {
  validateAssertion,
  rejectAssertion,
  addHumanResponse,
...
researchApi.post('/sessions', async (c) => {
  try {
    ...
    const result = await researchSessionManager.createSession({
      entityId,
      entityName,
      entityUrl,
      projectId,
      researcherName,
      categories: categories as ResearchCategory[],
      mode,
    });
...
researchApi.post('/sessions/:sessionId/pause', async (c) => {
  try {
    ...
    await researchSessionManager.pauseSession(sessionId);
...
researchApi.post('/sessions/:sessionId/resume', async (c) => {
  try {
    ...
    await researchSessionManager.resumeSession(sessionId);
...
researchApi.post('/sessions/:sessionId/cancel', async (c) => {
  try {
    ...
    await researchSessionManager.cancelSession(sessionId);
...
researchApi.post('/sessions/:sessionId/tasks/:taskId/retry', async (c) => {
  try {
    ...
    const session = researchSessionManager.getSession(sessionId);
    if (session) {
      researchSessionManager.sendMessage(
        sessionId,
        `[RETRY] Please retry the ${task.category} research task (task ID: ${taskId})`
      );
    }
...
researchApi.get('/entities/:entityId/status', async (c) => {
  try {
    ...
    const latestSession = await prisma.researchSession.findFirst({
      where: { entityId },
      orderBy: { createdAt: 'desc' },
      include: {
        tasks: true,
    ...
    const coverageBySchema: Record<string, { hasExtraction: boolean; lastExtracted?: Date; isStale: boolean }> = {
...
researchApi.post('/feedback', async (c) => {
  try {
    ...
    switch (feedbackType) {
      case FeedbackType.VALIDATE:
        const validated = await validateAssertion(assertionId, validatedBy);
        result = {
          success: true,
          assertionId,
          action: 'validated',
      ...
      case FeedbackType.REJECT:
        const rejected = await rejectAssertion(assertionId, validatedBy, comment);
        result = {
          success: true,
          assertionId,
          action: 'rejected',
      ...
      case FeedbackType.CHALLENGE:
        await addHumanResponse(assertionId, comment || 'Challenge raised', validatedBy, {
      ...
      case FeedbackType.REFINE:
        await addHumanResponse(assertionId, comment || 'Refinement requested', validatedBy, {
      ...
      case FeedbackType.REQUEST_EVIDENCE:
        await addHumanResponse(
          assertionId,
          comment || 'Additional evidence requested',
          validatedBy,
          {}
        );
...
researchApi.post('/assertions/:id/validate', async (c) => {
  try {
    ...
    const assertion = await validateAssertion(assertionId, validatedBy);
...
researchApi.post('/assertions/:id/reject', async (c) => {
  try {
    ...
    const assertion = await rejectAssertion(assertionId, validatedBy, reason);
...
researchApi.post('/assertions/:id/challenge', async (c) => {
  try {
    ...
    const assertion = await addHumanResponse(
      assertionId,
      comment || 'Challenge raised',
      validatedBy,
      { partiallyValidated: false }
    );
...
researchApi.post('/assertions/:id/request-evidence', async (c) => {
  try {
    ...
    const assertion = await addHumanResponse(assertionId, message, requestedBy, {});
