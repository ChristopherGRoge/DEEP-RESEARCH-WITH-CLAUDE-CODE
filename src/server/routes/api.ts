...
api.get('/assertions/by-project', async (c) => {
  ...
  if (projectId) {
    ...
    const sortedEntities = Array.from(byEntity.values()).sort((a, b) =>
...
api.get('/entities/tree/:projectId', async (c) => {
  ...
  const categoryWeights: Record<string, { weight: number; normalizedWeight: number; totalBuzz: number; avgBuzz: number }> = {};
  let maxWeight = 0;
  let minWeight = Infinity;

  for (const [key, categoryEntities] of grouped.entries()) {
    const entityCount = categoryEntities.length;
    const buzzScores = categoryEntities.map((e: any) => e.buzzScore || 0);
    const totalBuzz = buzzScores.reduce((sum: number, b: number) => sum + b, 0);
  ...
  const treeData = {
    ...
    children: sortedEntries.map(([key, catEntities]) => ({
      ...
      children: catEntities
        ...
        .sort((a: any, b: any) => (b.buzzScore || 0) - (a.buzzScore || 0))
        .map((e: any) => ({
          name: e.name,
          ...
          logoUrl: e.logoSvgContent ? null : (e.logoPath ? '/' + e.logoPath.replace(/^.*\/logos\//, 'logos/') : e.logoUrl),
...
api.get('/entities/:id/full', async (c) => {
  ...
  return c.json({
    ...
    data: {
      ...
      entityType: entity.entityType,
      url: entity.url,
      logoUrl: entity.logoSvgContent ? null : (entity.logoPath ? '/' + entity.logoPath.replace(/^.*\/logos\//, 'logos/') : entity.logoUrl),
      logoSvgContent: entity.logoSvgContent,
      logoPath: entity.logoPath ? '/' + entity.logoPath.replace(/^.*\/logos\//, 'logos/') : null,
