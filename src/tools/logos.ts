...
export interface LogoCandidate {
  ...
  confidence: number;   // 0-1, how likely this is the official logo
  reason: string;       // Why this was identified as a logo
}
...
export interface LogoDownloadResult {
  ...
  format?: string;
  sizeBytes?: number;
  svgContent?: string;  // Raw SVG markup if format is SVG
  error?: string;
}
...
export interface LogoSaveInput {
  ...
  logoFormat?: string;
  svgContent?: string;  // Raw SVG markup (if already fetched)
  download?: boolean;   // Whether to download locally
}
...
export interface LogoSaveResult {
  ...
  logoPath?: string;
  logoFormat?: string;
  hasSvgContent?: boolean;  // Whether SVG was stored inline
  error?: string;
}
...
export async function closeLogoBrowser(): Promise<void> {
 ...
 */
export async function searchForLogo(entityId: string): Promise<LogoSearchResult> {
  // Get entity
    ...
    for (const pageUrl of pagesToSearch) {
      try {
        ...
        const images = await page.evaluate(() => {
          const imgs: Array<{ src: string; alt: string; classes: string; id: string; width: number; height: number }> = [];
 ...
 */
export async function verifyLogoUrl(url: string): Promise<LogoVerifyResult> {
 ...
 */
export async function saveLogo(input: LogoSaveInput): Promise<LogoSaveResult> {
 ...
 */
async function fetchLogoInternal(entityId: string): Promise<FetchLogoResult> {
  // Search for logos
...
export async function fetchLogo(
  entityId: string,
  timeoutMs: number = FETCH_LOGO_TIMEOUT_MS
): Promise<FetchLogoResult> {
  // Create timeout promise
  const timeoutPromise = new Promise<FetchLogoResult>((_, reject) => {
    setTimeout(() => {
 ...
 */
export async function getEntitiesWithoutLogos(projectId?: string): Promise<Array<{
  id: string;
  name: string;
  url: string | null;
  hasUrl: boolean;
 ...
 */
export async function getLogoSummary(projectId: string): Promise<{
  total: number;
  withLogo: number;
  withoutLogo: number;
  verified: number;
  downloaded: number;
  coverage: number;
  entitiesNeedingLogos: Array<{ id: string; name: string; hasUrl: boolean }>;
}> {
  const entities = await prisma.entity.findMany({
    where: { projectId },
    select: {
 ...
 */
export async function verifyEntityLogo(
  entityId: string,
  verifiedBy: string
 ...
 */
export async function clearLogo(entityId: string): Promise<{ success: boolean; error?: string }> {
  const entity = await prisma.entity.findUnique({
    where: { id: entityId },
  });
 ...
 */
export async function getLogoInline(entityId: string): Promise<{
  success: boolean;
  entityId: string;
  entityName: string;
  format?: string;
...
function getImageFormat(url: string): string | null {
...
function scoreLogoCandidate(
  img: { src: string; alt: string; classes: string; id: string; width: number; height: number },
  entityName: string,
  pageUrl: string
): number {
  let score = 0;
  ...
  if (LOGO_PATTERNS.some(p => p.test(src))) score += 0.3;
  if (LOGO_PATTERNS.some(p => p.test(alt))) score += 0.2;
  if (LOGO_PATTERNS.some(p => p.test(classes))) score += 0.2;
  if (LOGO_PATTERNS.some(p => p.test(id))) score += 0.2;

  // Check for entity name
  if (src.includes(entityLower.replace(/\s+/g, ''))) score += 0.3;
  if (alt.includes(entityLower)) score += 0.2;

  // STRONGLY prefer SVG (can be stored inline, scales infinitely)
  if (src.endsWith('.svg')) score += 0.5;

  // Penalize very small or very large images
  if (img.width > 0 && img.height > 0) {
    if (img.width < 20 || img.height < 20) score -= 0.3;
    if (img.width > 1000 || img.height > 1000) score -= 0.2;
  }

  // Boost if on press/brand page
  if (pageUrl.includes('press') || pageUrl.includes('brand')) score += 0.2;

  // Penalize common non-logo patterns
  if (src.includes('avatar') || src.includes('profile')) score -= 0.5;
  if (src.includes('banner') || src.includes('hero')) score -= 0.3;
  if (src.includes('screenshot') || src.includes('demo')) score -= 0.3;

  return Math.min(1, Math.max(0, score));
}

function getScoreReason(
  img: { src: string; alt: string; classes: string; id: string },
  entityName: string
): string {
  ...
  if (LOGO_PATTERNS.some(p => p.test(src))) reasons.push('logo in filename');
  if (LOGO_PATTERNS.some(p => p.test(alt))) reasons.push('logo in alt text');
  if (LOGO_PATTERNS.some(p => p.test(classes))) reasons.push('logo class');
  if (src.includes(entityName.toLowerCase())) reasons.push('entity name in URL');
  if (src.endsWith('.svg')) reasons.push('SVG format');

  return reasons.join(', ') || 'potential match';
}

function isLogoLink(url: string, entityName: string): boolean {
  const lower = url.toLowerCase();
  const entityLower = entityName.toLowerCase().replace(/\s+/g, '');

  // Must be an image file
  if (!getImageFormat(url)) return false;

  // Check for logo indicators
  if (LOGO_PATTERNS.some(p => p.test(lower))) return true;
  if (lower.includes(entityLower) && getImageFormat(url)) return true;

  return false;
}

function deduplicateCandidates(candidates: LogoCandidate[]): LogoCandidate[] {
