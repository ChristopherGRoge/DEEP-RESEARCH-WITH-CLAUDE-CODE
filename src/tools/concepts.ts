...
export async function createConcept(input: CreateConceptInput) {
  ...
  const concept = await prisma.categoryConcept.upsert({
    where: {
      categoryId_name: {
...
export async function getConcept(conceptId: string) {
  const concept = await prisma.categoryConcept.findUnique({
    where: { id: conceptId },
    include: {
      category: { select: { id: true, name: true, displayName: true } },
      entityLinks: {
 ...
 */
export async function listConcepts(input: ListConceptsInput) {
  ...
  const concepts = await prisma.categoryConcept.findMany({
    where,
    include: {
      _count: { select: { entityLinks: true } },
    },
 ...
 */
export async function updateConcept(conceptId: string, input: UpdateConceptInput) {
  const existing = await prisma.categoryConcept.findUnique({
    where: { id: conceptId },
  });
  ...
  const updated = await prisma.categoryConcept.update({
    where: { id: conceptId },
    data: {
...
export async function deleteConcept(conceptId: string) {
  ...
  await prisma.categoryConcept.delete({
    where: { id: conceptId },
  });
 ...
 */
export async function linkConcept(input: LinkConceptInput) {
  ...
  const link = await prisma.conceptEntityLink.upsert({
    where: {
      conceptId_entityId: {
 ...
 */
export async function unlinkConcept(conceptId: string, entityId: string) {
  const existing = await prisma.conceptEntityLink.findUnique({
    where: {
      conceptId_entityId: {
        conceptId,
        entityId,
  ...
  await prisma.conceptEntityLink.delete({
 ...
 */
export async function getEntityConcepts(entityId: string) {
  const entity = await prisma.entity.findUnique({
    where: { id: entityId },
  ...
  const byType: Record<string, typeof links> = {};
 ...
 */
export async function getConceptEntities(conceptId: string) {
  const concept = await prisma.categoryConcept.findUnique({
    where: { id: conceptId },
 ...
 */
export async function getCategoryConceptMap(categoryId: string) {
  const category = await prisma.discoveryCategory.findUnique({
    where: { id: categoryId },
  ...
  const entityConceptMap: Record<string, Array<{ conceptId: string; conceptName: string; conceptType: string; strength: number }>> = {};
  for (const concept of concepts) {
