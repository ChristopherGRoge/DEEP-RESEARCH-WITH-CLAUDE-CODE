 ...
 */
export async function generateValidationDashboard(options: ValidationDashboardOptions = {}) {
  ...
  const rejectedAssertions = await prisma.assertion.findMany({
    where: {
  ...
  const grouped = {
    CRITICAL: assertions.filter(a => a.criticality === AssertionCriticality.CRITICAL),
    HIGH: assertions.filter(a => a.criticality === AssertionCriticality.HIGH),
    MEDIUM: assertions.filter(a => a.criticality === AssertionCriticality.MEDIUM),
    LOW: assertions.filter(a => a.criticality === AssertionCriticality.LOW),
  };
...
function generateHtml(grouped: Record<string, any[]>, rejected: any[], validatorName: string): string {
  const criticalityColors: Record<string, { bg: string; border: string; text: string }> = {
  ...
  const renderAssertion = (assertion: any, criticality: string) => {
    const colors = criticalityColors[criticality];
    const sources = assertion.sources.map((as: any) => ({
      url: as.source.url,
      quote: as.quote,
      ...
      <div class="assertion-card" style="border-left: 4px solid ${colors.border}; background: ${colors.bg};">
        ...
        ${assertion.reasoning.length > 0 ? `
          <div class="reasoning">
            ...
            ${assertion.reasoning.map((r: any) => `<p>${escapeHtml(r.content)}</p>`).join('')}
          </div>
        ` : ''}
  ...
  const renderRejected = (assertion: any) => {
