...
export interface BuzzComponents {
  marketPresence: number;    // 0-1: GitHub stars, employee size, enterprise tier
  developerActivity: number; // 0-1: Commits, contributors, releases
...
function normalizeStars(stars: number | null): number {
  ...
  if (stars >= 50000) return 1.0;
  if (stars >= 10000) return 0.8 + (0.2 * (stars - 10000) / 40000);
  if (stars >= 1000) return 0.6 + (0.2 * (stars - 1000) / 9000);
  if (stars >= 100) return 0.4 + (0.2 * (stars - 100) / 900);
 ...
 */
function normalizeContributors(contributors: number | null): number {
  if (!contributors || contributors <= 0) return 0;
  if (contributors >= 500) return 1.0;
  if (contributors >= 200) return 0.7 + (0.3 * (contributors - 200) / 300);
  if (contributors >= 50) return 0.5 + (0.2 * (contributors - 50) / 150);
  if (contributors >= 10) return 0.3 + (0.2 * (contributors - 10) / 40);
 ...
 */
function normalizeEmployeeCount(employeeCount: string | number | null): number {
  if (employeeCount === null || employeeCount === undefined) return 0;

  // Handle numeric values directly
  if (typeof employeeCount === 'number') {
    if (employeeCount >= 1000) return 0.9;
    if (employeeCount >= 200) return 0.7;
    if (employeeCount >= 50) return 0.5;
    if (employeeCount >= 10) return 0.3;
    return 0.2;
  }
  ...
  if (lower.includes('1000') || lower.includes('1k')) return 0.9;
  if (lower.includes('500') || lower.includes('201-')) return 0.7;
  if (lower.includes('200') || lower.includes('51-')) return 0.5;
  if (lower.includes('50') || lower.includes('11-')) return 0.3;
  if (lower.includes('10') || lower.includes('1-')) return 0.2;

  // Try to parse number from string
  const match = lower.match(/(\d+)/);
  if (match) {
    const num = parseInt(match[1], 10);
    if (num >= 1000) return 0.9;
    if (num >= 200) return 0.7;
    if (num >= 50) return 0.5;
    if (num >= 10) return 0.3;
    return 0.2;
  }
 ...
 */
function normalizeFunding(totalRaised: string | null, lastRound: string | null): number {
  // Check funding round first (more reliable)
  if (lastRound) {
    const round = lastRound.toLowerCase();
    if (round.includes('ipo') || round.includes('public')) return 0.95;
    if (round.includes('series d') || round.includes('series e') || round.includes('series f')) return 0.9;
    if (round.includes('series c')) return 0.85;
    if (round.includes('series b')) return 0.7;
    if (round.includes('series a')) return 0.55;
    if (round.includes('seed')) return 0.4;
    if (round.includes('angel') || round.includes('pre-seed')) return 0.3;
  }
  ...
  if (totalRaised) {
    ...
    if (millionMatch) {
      ...
      if (millions >= 500) return 0.9;
      if (millions >= 100) return 0.8;
      if (millions >= 50) return 0.7;
      if (millions >= 20) return 0.6;
      if (millions >= 10) return 0.5;
      if (millions >= 5) return 0.4;
      return 0.3;
    }
  }
 ...
 */
function normalizeRecency(daysSinceActivity: number): number {
  if (daysSinceActivity <= 7) return 1.0;
  if (daysSinceActivity <= 30) return 0.9;
  if (daysSinceActivity <= 90) return 0.7;
  if (daysSinceActivity <= 180) return 0.5;
  if (daysSinceActivity <= 365) return 0.3;
  return 0.1;
}
 ...
 */
async function calculateMarketPresence(
  entity: any,
  companyData: any
 ...
 */
async function calculateDeveloperActivity(
  entity: any
): Promise<{ score: number; missing: string[] }> {
  const missing: string[] = [];
  const signals: number[] = [];
 ...
 */
async function calculateFundingSignal(
  companyData: any
): Promise<{ score: number; missing: string[] }> {
  const missing: string[] = [];
 ...
 */
async function calculateMentionVelocity(
  entityId: string
): Promise<{ score: number; missing: string[] }> {
  const missing: string[] = [];
  ...
  const totalMentions = assertions.reduce((sum: number, a: { mentionCount: number }) => sum + a.mentionCount, 0);

  // Average source spread (how many different sources mention claims)
  const avgSourceSpread = assertions.reduce((sum: number, a: { sourceSpread: number }) => sum + a.sourceSpread, 0) / assertions.length;

  // Recent assertion activity (claims added recently = active buzz)
  const recentAssertions = assertions.filter(
    (a: { createdAt: Date }) => daysSince(a.createdAt) <= 30
  ).length;

  const signals: number[] = [];

  // Mention count signal
  if (totalMentions > 50) signals.push(1.0);
  else if (totalMentions > 20) signals.push(0.8);
  else if (totalMentions > 10) signals.push(0.6);
  else if (totalMentions > 5) signals.push(0.4);
  else signals.push(0.2);

  // Source spread signal
  if (avgSourceSpread > 3) signals.push(1.0);
  else if (avgSourceSpread > 2) signals.push(0.8);
  else if (avgSourceSpread > 1.5) signals.push(0.6);
  else signals.push(0.3);

  // Recent activity signal
  if (recentAssertions > 10) signals.push(1.0);
  else if (recentAssertions > 5) signals.push(0.8);
  else if (recentAssertions > 2) signals.push(0.6);
  else if (recentAssertions > 0) signals.push(0.4);
  else signals.push(0.2);
 ...
 */
async function calculateResearchDepth(
  entityId: string
): Promise<{ score: number; missing: string[] }> {
  const missing: string[] = [];

  // Count assertions by status
  const assertions = await prisma.assertion.groupBy({
    by: ['status'],
    where: { entityId },
    _count: true,
  });
  ...
  const totalAssertions = assertions.reduce((sum: number, a: { _count: number }) => sum + a._count, 0);
  const validatedAssertions = assertions.find((a: { status: string }) => a.status === 'EVIDENCE')?._count || 0;
  const extractionTypes = new Set(extractions.map((e: { schemaType: string }) => e.schemaType));

  const signals: number[] = [];

  // Assertion coverage
  if (totalAssertions >= 20) signals.push(1.0);
  else if (totalAssertions >= 10) signals.push(0.8);
  else if (totalAssertions >= 5) signals.push(0.6);
  else if (totalAssertions > 0) signals.push(0.4);
  else {
    missing.push('assertions');
    signals.push(0);
  }
 ...
 */
export async function calculateBuzzScore(input: {
  entityId: string;
  ...
  const companyExtraction = entity.extractions.find(
    (e: { schemaType: string }) => e.schemaType === 'company'
  );
  const companyData = companyExtraction?.data as any;
  ...
  await prisma.entity.update({
    ...
    data: {
      ...
      buzzComponents: components as unknown as Record<string, number>,
 ...
 */
export async function calculateProjectBuzzScores(input: {
  projectId: string;
  ...
  const results: BuzzCalculationResult[] = [];
 ...
 */
export async function setBuzzOverride(input: {
  entityId: string;
  buzzOverride: number;
  reason: string;
}): Promise<{
 ...
 */
export async function clearBuzzOverride(input: {
  entityId: string;
 ...
 */
export async function getEntitiesByBuzzScore(input: {
  projectId: string;
  limit?: number;
  minBuzz?: number;
  categoryId?: string;
  ...
  return {
    success: true,
    entities: entities.map((e: typeof entities[0]) => {
      const components = e.buzzComponents as unknown as BuzzComponents | null;
      const componentValues = components ? Object.values(components) : [];
      const filledCount = componentValues.filter((v: number) => v > 0).length;
