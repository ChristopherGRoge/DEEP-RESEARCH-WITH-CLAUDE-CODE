...
function testExtractDiscoveries() {
  ...
  const discoveries = extractDiscoveries(mockTweets);
  ...
  const falsePositives = discoveries.filter(d =>
    commonWords.includes(d.name)
  );

  if (falsePositives.length > 0) {
    console.warn(`⚠ Warning: Found ${falsePositives.length} potential false positives:`,
      falsePositives.map(d => d.name));
  } else {
...
function testDeduplication() {
  ...
  const discoveries = extractDiscoveries(duplicateTweets);
  ...
  const cursorMentions = discoveries.filter(d =>
    d.name.toLowerCase().includes('cursor')
  );
...
export { runAllTests, testExtractDiscoveries, testConfidenceScoring, testDeduplication };
